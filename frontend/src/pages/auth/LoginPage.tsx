...
export const LoginPage: React.FC = () => {
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      await demoLogin();
      setLoginSuccess(true);
      success('Logged in with Demo Account!', 'Welcome');
      setTimeout(() => {
        navigate(from, { replace: true });
      }, 500);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Failed to launch demo mode.';
      setServerError(msg);
    } finally {
      setIsSubmitting(false);
    }
    ...
    <div className="bg-white p-6 sm:p-8 rounded-2xl border border-edge shadow-card w-full">
      ...
      <div className="mt-4 pt-4 border-t border-edge text-center">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-2 px-3 text-xs font-semibold text-brand-700 bg-brand-50 hover:bg-brand-100 rounded-xl transition-colors border border-brand-200"
        >
